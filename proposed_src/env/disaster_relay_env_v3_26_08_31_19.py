"""재난 릴레이 드론 환경 (v3, 26_08_31_19) — 계층 제어용.

상위가 드론별 하위 목표(배송지 / 복귀 / 유지)를 지정하면 하위는 2D 추력만 담당한다.
v2에서 효과가 측정되지 않은 기제(중계 쉐이핑·가중 재분배·각도 행동·전체취소 투영)는 제거했다.
"""
...
class DisasterRelayDroneEnv:
	...
	def manager_obs(self):
		"""상위 관측: 자기 상태, 후보 목적지 상대 정보, 동료 상태. (드론, 관측차원)"""
		h = self._hop_depth()
		out = []
		for i in range(self.num_drones):
