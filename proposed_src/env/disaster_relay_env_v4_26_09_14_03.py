"""재난 릴레이 드론 환경 (v4, 26_09_14_03) — 중계 지점 선택 행동.

v3_26_09_07_17 대비 한 가지를 바꿨다. 상위 행동 n_cand+1 이 '현 위치 유지'가 아니라
'제어 센터와 프런티어를 잇는 선분 위의 중계 슬롯으로 이동해 정박'을 뜻한다.
v3에는 중계 드론이 어디에 설지 고르는 행동이 아예 없어, 체인은 배송하러 가던 드론이
우연히 적당한 자리에 멈춰야 만들어졌다. 행동 수(K=7)와 관측 차원은 그대로 두어
기존 하위 가중치를 그대로 재사용한다.
"""
...
class DisasterRelayDroneEnv:
	...
	def __init__(self, map_path=DEFAULT_MAP, render_mode=None, comm_range=300.0,
	             ...
	             incomplete_penalty=0.0, arrive_once=True, relay_beta=0.9,
	             relay_hold=0.02):
		self.num_drones = 4
		self.num_dests = 50
