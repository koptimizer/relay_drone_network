"""재난 릴레이 드론 환경 (v1, 26_08_25_04).

26_08_24_22 대비: 이동 차단 타이브레이크 편향 제거, 차단 페널티를 보정 크기에 비례화,
중계 도달 범위(홉) 지표와 마지막 배송 시각 추가.
"""
...
class DisasterRelayDroneEnv:
	...
	def _enforce_connectivity(self, proposed_pos, intended_to_move):
		...
		while True:
			...
			# 동점일 때 낮은 인덱스를 고정 선택하면 사슬을 붙잡는 앵커 드론이 페널티를
			# 독식한다(0번이 265회, 나머지 0회로 재현됨). 동점은 무작위로 깬다.
			cnts = np.array([
				self._bfs_connected(
					np.where(np.where(np.arange(self.num_drones) == m, False, valid)[:, None],
					         proposed_pos, self.drones_pos)
				).sum()
				for m in movers
			])
			best = int(self.rng.choice(movers[cnts == cnts.max()]))
			valid[best] = False
		...
		for i in range(self.num_drones):
			if intended_to_move[i]:
				if valid_movement[i]:
					...
					# 페널티를 취소된 변위 크기에 비례시킨다. 제자리 명령(중계 대기)은
					# 보정량이 0이므로 처벌받지 않는다.
					corr = np.linalg.norm(proposed_pos[i] - self.drones_pos[i]) / self.max_speed
					if corr > 1e-6:
						self.blocked[i] += 1
						rewards[i] -= 0.1 * corr
