...
P.add_argument("--no-progress-limit", type=int, default=500,
               help="무배송 강제 종료. 정상 배송 간격 최대 109의 5배 여유")
P.add_argument("--warmup-episodes", type=int, default=300,
               help="이 에피소드까지는 무배송 한도를 완화해 재적재 사이클을 경험시킨다")
P.add_argument("--warmup-no-progress", type=int, default=500)
P.add_argument("--gamma", type=float, default=0.997, help="할인율 (지평 = 1/(1-gamma))")
P.add_argument("--fixed-instance", action="store_true",
               help="v4까지처럼 고정 지도 한 장으로 학습 (절제용). 기본은 에피소드마다 새 인스턴스")
P.add_argument("--random-cc", action="store_true", help="제어 센터 위치도 에피소드마다 무작위화")
P.add_argument("--n-far", type=int, default=0, help="후보 중 먼 곳 수 (2로 두면 -2.1, 기본 0)")
P.add_argument("--commit-max", type=int, default=150, help="목표 유지 최대 스텝")
P.add_argument("--commit", action="store_true", help="목표 유지 마스크 (켜면 -6.5, alpha 폭주. 기본 off)")
P.add_argument("--learned-worker", action="store_true",
               help="하위를 학습 정책으로. 기본은 직진 제어 (v4: 완주 0-2% -> 14-38%)")
...
A.straight_worker = A.straight_worker or not A.learned_worker
GAMMA = A.gamma
...
def _mgr_act(env, manager, device, deterministic):
	...
	with torch.no_grad():
		act, probs, _ = manager(mo, action_mask(env, device, A.no_relay, A.commit))
...
def main():
	...
	kw = dict(comm_range=A.comm_range, cluster_penalty=not A.no_cluster_penalty,
	          ...
	          relay_hold=A.relay_hold, n_far=A.n_far, commit_max=A.commit_max)
	...
	print(f"상한={A.max_steps} 교착한도={A.deadlock_limit} 무배송한도={A.no_progress_limit} "
	      ...
	      f"중계정박={A.relay_hold} 인스턴스={'고정' if A.fixed_instance else '무작위'}"
	      f"{'(CC무작위)' if A.random_cc else ''} 목표유지={A.commit_max if A.commit else 'off'} "
	      f"하위={'학습' if not A.straight_worker else '직진'}", flush=True)
	...
	inst_rng = np.random.default_rng(A.seed + 1000)   # 학습 인스턴스 추출 전용
	...
	for ep in range(start_ep, A.max_episodes + 1):
		...
		# v4까지는 고정 지도 한 장으로만 학습해 인스턴스를 암기했다. 에피소드마다 새로 뽑는다.
		if not A.fixed_instance:
			env.cc_pos, env.dests_pos = sample_instance(
				env.num_dests, seed=int(inst_rng.integers(0, 2 ** 31 - 1)),
				num_drones=env.num_drones, comm_range=env.comm_range,
				cc_pos="random" if A.random_cc else None)
		obs = env.reset()
		mo = env.manager_obs()
		...
		while not done and t < A.max_steps:
			...
			nmask = action_mask(env, dev, A.no_relay, A.commit).cpu().numpy()
			...
			if len(buf) > WARMUP:
				...
				if bh is not None:
					...
					ent = -(p * lp).sum(-1).mean()
					# 유효 행동 수 기준 목표 엔트로피. 고정 0.6*log(K)는 마스크로 선택지가 줄면
					# 달성 불가능해 alpha가 폭주한다.
					m_target_ent = (0.6 * torch.log(bh["mask2"].float().sum(-1).clamp_min(1.0))).mean()
					al = -(m_alpha * (m_target_ent - ent).detach()).mean()
					m_alpha_opt.zero_grad(); al.backward(); m_alpha_opt.step()
					m_alpha.data.clamp_(np.log(1e-3), np.log(10.0))
