...
def main():
	...
	p.add_argument("--out", default="eval_v3_26_09_11_11")
	...
	for i, wck in enumerate(args.worker):
		...
		wf = learned_worker(os.path.join(ROOT, wck), wo, dev, args.stochastic)
		if i < len(args.manager) and args.manager[i]:
			mf = learned_manager(os.path.join(ROOT, args.manager[i]), mo, K, dev, args.stochastic)
