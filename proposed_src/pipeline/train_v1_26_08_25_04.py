...
# relay_w는 중계 사슬 형성을 유도하는 potential 쉐이핑 가중치 (0.0이면 비활성).
_p = argparse.ArgumentParser()
_p.add_argument("--tag", default="v1_26_08_25_04", help="실행 태그 (runs/·weights/ 하위 폴더명)")
...
CSV_COLS = [
	...
	"hop2plus", "max_reach", "t_last_deliv", "comm_loss", "alpha", "loss_critic", "loss_actor", "env_steps", "wall_sec",
]
...
if __name__ == "__main__":
	...
	for episode in range(1, MAX_EPISODES + 1):
		...
		writer.add_scalar('Relay/Hop2PlusRatio', st["hop2plus"], episode)
		writer.add_scalar('Relay/MaxReach', st["max_reach"], episode)
		writer.add_scalar('Delivery/LastDeliveryStep', st["t_last_deliv"], episode)
		...
		print(
			...
			f"재적재 {st['reloads']} | 2홉+ {st['hop2plus']:.2f} | 도달 {st['max_reach']:4.0f} | 차단 {st['blocked']:4d}",
			flush=True
		)
