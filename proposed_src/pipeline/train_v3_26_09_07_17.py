...
POLICY_DELAY, MAX_STEPS, CKPT_EVERY = 2, 10000, 10
...
P.add_argument("--max-steps", type=int, default=10000, help="에피소드 스텝 상한")
P.add_argument("--deadlock-limit", type=int, default=100)
P.add_argument("--no-progress-limit", type=int, default=200)
P.add_argument("--warmup-episodes", type=int, default=300,
               help="이 에피소드까지는 무배송 한도를 완화해 재적재 사이클을 경험시킨다")
P.add_argument("--warmup-no-progress", type=int, default=1000)
P.add_argument("--gamma", type=float, default=0.997, help="할인율 (지평 = 1/(1-gamma))")
P.add_argument("--arrive-every", action="store_true",
               help="도달 보너스를 26_09_04_15처럼 반경 안 매 스텝 지급 (회귀 절제용)")
...
def action_mask(env, device):
	...
	for i in range(env.num_drones):
		...
		# 복귀는 항상 허용한다. 적재량이 가득 찬 드론도 사슬이 팽팽할 때 제어 센터 쪽으로
		# 물러나 여유를 만들어야 하는데, 이전 버전은 이 경로를 막아 교착에서 못 빠져나왔다.
		m[i, env.n_cand] = True
		if not m[i].any():
...
def main():
	...
	kw = dict(comm_range=A.comm_range, cluster_penalty=not A.no_cluster_penalty,
	          ...
	          incomplete_penalty=A.incomplete_penalty, arrive_once=not A.arrive_every)
	...
	print(f"상한={A.max_steps} 교착한도={A.deadlock_limit} 무배송한도={A.no_progress_limit} "
	      ...
	      f"도달보너스={'매스텝' if A.arrive_every else '목표당1회'}", flush=True)
