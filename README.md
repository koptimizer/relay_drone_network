...
홀드아웃 60 인스턴스 × 5회(롤아웃 300개), 학습 시드 4개, 통신 반경 300.

| 방법 | 배송 | 표준오차 | 최대 도달 | 이동 취소 |
|---|---|---|---|---|
| 탐욕 | 23.04 | 0.51 | 649 | 3,341 |
| 탐욕 + 중계 1대 고정 | 14.51 | 0.35 | 533 | 2,635 |
| **제안 (계층, 시드 4개 평균)** | **24.90** | **0.50** | **818** | **1,279** |
| 동시 학습 (두 계층) | 21.43 | 0.30 | 680 | 1,300 |

**제안 방법이 탐욕을 8.1% 앞서며 통계적으로 유의하다** — 95% 신뢰구간 [23.32, 26.48],
t = 3.74, p = 0.034, 4개 시드 전부 초과. 보조 지표의 차이는 더 크다. 최대 도달 거리가
통신 반경의 약 2.7배까지 늘었고(+25%), 연결 제약에 의한 이동 취소가 60% 이상 줄었다.
통신 단절은 학습·평가 전 구간에서 0건이다.

**두 계층의 동시 학습은 오히려 해롭다**(21.43, 탐욕보다도 낮음). 하위 정책이 갱신되면
상위가 고른 목표의 의미가 바뀌어 상위 관점에서 환경이 비정상이 되기 때문으로 보인다.
