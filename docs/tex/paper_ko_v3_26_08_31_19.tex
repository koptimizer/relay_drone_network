...
통신 반경 300, 목적지 50곳, 드론 4대 설정에서 홀드아웃 60개 인스턴스를 5회씩 굴려
롤아웃 300개로 평가하고, 학습을 서로 다른 시드 4개로 반복하였다. 제안 방법은 배송 24.90건
(시드 간 표준오차 0.50, 95\% 신뢰구간 [23.32, 26.48])으로 탐욕 휴리스틱 23.04건을 넘어섰으며
그 차이는 통계적으로 유의하였다($t=3.74$, $p=0.034$, 4개 시드 모두 초과). 보조 지표의 차이는
더 크다. 제어 센터로부터의 최대 도달 거리는 649에서 807--830으로 25\% 증가하여 직접 통신
반경의 약 2.7배에 이르렀고, 연결 제약에 의한 이동 취소는 3,341회에서 1,051--1,434회로 60\%
이상 감소하였다. 두 계층을 동시에 미세조정하는 구성은 21.43건으로 오히려 열화하였는데, 하위
정책의 변화가 상위가 선택한 목표의 의미를 바꾸어 상위 관점에서 환경이 비정상이 되기 때문으로
해석된다. 통신 단절은 학습 및 평가 전 구간에서 한 건도 발생하지 않았다.
\end{abstract}
...
탐욕 휴리스틱 대비 배송량이 8.1\% 높으며 도달 범위가 25\% 넓고 제약 충돌이 60\% 이상 적음을
보였다. 아울러 두 계층의 동시 학습이 오히려 성능을 떨어뜨림을 보여 단계적 학습의 필요성을 밝혔다.
...
\begin{table}[h]
\centering
\caption{주 결과 (홀드아웃 60 인스턴스 $\times$ 5회 = 롤아웃 300개, 통신 반경 300)}
\label{tab:main}
\begin{tabular}{lrrrrrr}
\toprule
방법 & 배송 & 표준편차 & 표준오차 & 재적재 & 최대 도달 & 이동 취소 \\
\midrule
탐욕 & 23.04 & 8.89 & 0.51 & 2.11 & 649 & 3{,}341 \\
탐욕 + 중계 1대 고정 & 14.51 & 6.11 & 0.35 & 1.06 & 533 & 2{,}635 \\
제안 (시드 0) & 24.72 & 6.38 & 0.37 & 2.37 & 807 & 1{,}324 \\
제안 (시드 1) & \textbf{25.95} & 6.61 & 0.38 & 2.92 & 826 & 1{,}308 \\
제안 (시드 2) & 23.61 & 6.86 & 0.40 & 2.49 & 830 & 1{,}434 \\
제안 (시드 3) & 25.32 & 5.68 & 0.33 & 2.50 & 808 & 1{,}051 \\
\textbf{제안 (시드 4개 평균)} & \textbf{24.90} & --- & \textbf{0.50} & 2.57 & \textbf{818} & \textbf{1{,}279} \\
동시 학습 (두 계층) & 21.43 & 5.20 & 0.30 & 2.30 & 680 & 1{,}300 \\
\bottomrule
\end{tabular}
\end{table}

학습 시드 4개를 표본으로 보면 제안 방법의 평균은 24.90건, 시드 간 표준편차는 1.00, 표준오차는
0.50이다. 95\% 신뢰구간 $[23.32,\,26.48]$은 탐욕의 23.04건을 포함하지 않으며, 단일표본 $t$검정
결과 $t=3.74$, $p=0.034$로 차이가 유의하다. 4개 시드가 모두 탐욕을 초과하였고 개선폭은 8.1\%다.

보조 지표의 차이는 더 크다. 제어 센터로부터의 최대 도달 거리가 649에서 807--830으로 증가하였는데,
통신 반경이 300임을 고려하면 중계 사슬이 직접 통신 반경의 약 2.7배까지 뻗었음을 뜻한다.
연결 제약에 의한 이동 취소는 3,341회에서 1,051--1,434회로 60\% 이상 감소하였다. 즉 제안 방법은
더 멀리 나아가면서 제약과는 덜 충돌한다. 재적재 역시 2.37--2.92회로 탐욕의 2.11회를 넘어섰다.

한편 두 계층을 동시에 미세조정한 구성은 21.43건으로 탐욕보다도 낮았다. 서로 다른 시드로
재학습하여도 같은 열화가 재현되었으며, 홉 깊이 비율(0.50)과 도달 거리(680) 또한 단계적 학습
구성보다 낮다. 하위 정책이 갱신되면 상위가 선택한 목표의 의미가 함께 바뀌므로, 상위 관점에서
환경이 비정상이 되어 학습된 할당이 무너지는 것으로 해석된다. 최종 구성에서는 동시 학습 단계를
제외한다.
...
상위 결정 주기 $H$에 대한 감도를 확인한 결과 $H=20$이 $H=10$보다 우세하였다. 재결정이 잦으면
하위 목표가 자주 바뀌어 하위 정책이 목표에 도달하기 전에 목표가 교체된다.
...
배송 처리량에서 제안 방법은 탐욕 휴리스틱을 8.1\% 앞섰으며 학습 시드 4개에서 모두 재현되었다
($p=0.034$). 개선폭 자체는 크지 않으나, 중계 네트워크 활용도의 차이는 뚜렷하다 --- 도달 거리
25\% 증가, 제약 충돌 60\% 이상 감소. 아울러 두 계층의 동시 학습이 오히려 성능을 떨어뜨림을
확인하여, 이 문제에서는 단계적 학습이 필요함을 밝혔다.
