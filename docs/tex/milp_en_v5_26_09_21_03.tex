...
\input{milp_results_v5_26_09_21_03}
...
to the chain slack. The measured ceiling on this hardware is between six and ten destinations
for three drones: with ten destinations the model has $10{,}587$ binaries and $99{,}091$ rows,
and two runs of ninety and of forty minutes returned no feasible plan at all. The model is a
reference point for small instances, not a solution method.

A certified lower bound on the continuous optimum would need the opposite discretisation: treat
each lattice cell as a region rather than a point and test movement and communication with the
\emph{minimum} distance between cells, $\max\{0, \|p_i - p_j\| - r_i - r_j\}$ with
$r_i = h/\sqrt{2}$ for a lattice cell and $r_i = 0$ for the control centre and the destinations.
Every continuous trajectory then maps to a feasible cell schedule, so the relaxed optimum bounds
the continuous one from below. Making that bound valid also requires relaxing the within-period
timing of deliveries and of the service freeze, which the present model fixes to period
boundaries; we leave the construction to future work rather than report a bound we cannot
certify. It also inherits the two
