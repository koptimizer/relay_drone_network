...
Over 30 held-out instances evaluated three times each at communication range 300 with 50
destinations and 4 drones, the proposed method served 23.97 demand points (standard error 0.67)
against 23.04 (standard error 1.01) for a greedy heuristic; the difference is not statistically
distinguishable. The gains lie elsewhere: dispersion across instances fell from 9.62 to 6.34,
maximum reach from the control center rose from 644 to 778, and movements cancelled by the
connectivity constraint fell by 59\%, from 3,338 to 1,373. No connectivity loss occurred during
any training episode or evaluation rollout.
\end{abstract}
...
framework separating destination selection from low-level maneuvering is proposed; it matches a
greedy heuristic in delivery volume while reducing dispersion by 34\% and extending reach by 21\%.
...
\begin{table}[h]
\centering
\caption{Main results (30 held-out instances $\times$ 3 repetitions, range 300)}
\label{tab:main}
\begin{tabular}{lrrrrrr}
\toprule
Method & Delivered & SD & SE & Reloads & Max reach & Moves cancelled \\
\midrule
Random policy & 3.7 & 2.5 & 0.26 & 0.0 & 372 & 17 \\
Greedy & 23.04 & 9.62 & 1.01 & 2.12 & 644 & 3{,}338 \\
Greedy + 1 fixed relay & 14.24 & 5.29 & 0.56 & 0.96 & 524 & 2{,}652 \\
Rule manager + learned worker & 23.09 & 9.25 & 0.97 & 2.50 & 695 & 1{,}303 \\
\textbf{Proposed (hierarchical, $H=20$)} & \textbf{23.97} & \textbf{6.34} & \textbf{0.67} & 2.34 &
\textbf{778} & \textbf{1{,}373} \\
Proposed (hierarchical, $H=10$) & 22.52 & 5.75 & 0.61 & 2.11 & 788 & 1{,}322 \\
\bottomrule
\end{tabular}
\end{table}

The proposed method served 23.97 demand points against 23.04 for greedy, but with standard errors
of 0.67 and 1.01 the difference of $0.93$ is not statistically significant. \textbf{In terms of
delivery throughput the proposed method should be reported as matching the greedy heuristic, not
as exceeding it.}

The significant differences appear elsewhere. Dispersion across instances fell from 9.62 to 6.34,
a 34\% reduction, indicating more consistent performance. Maximum reach from the control center
rose from 644 to 778; with a communication range of 300, this means the relay chain extended to
2.6 times the direct range. Movements cancelled by the connectivity constraint fell by 59\%, from
3,338 to 1,373. The proposed method therefore travels farther while colliding less with the
constraint.
...
Sensitivity to the manager decision period $H$ was examined; $H=20$ dominated $H=10$ (23.97 versus
22.52). Frequent re-decision replaces subgoals before the worker reaches them.
...
The results are qualified. In delivery throughput the proposed method is not statistically
distinguishable from a greedy heuristic, consistent with reports in the neural combinatorial
optimization literature that learned methods do not surpass well-tuned heuristics on solution
quality. The advantages observed are a 34\% reduction in performance dispersion and greater use of
the relay network---21\% more reach with 59\% fewer cancelled movements---which carry weight in a
disaster-response context independently of mean throughput.
