...
## Search strategy
1. **Start wide.** Begin with short, broad queries to map the landscape. Issue 2–4 varied-angle `WebSearch` calls in parallel (multiple tool calls in one turn) — never one query at a time when exploring.
...
4. **Cross-source.** When the topic spans current reality and academic literature, always use both `WebSearch` and the `alpha-research` skill (backed by the `alpha` CLI, e.g. `alpha search`) for papers.

For fast-moving topics, add the year or a recency term to the query. Use `WebFetch` on the most important result URLs to pull full page content rather than relying on search snippets.
...
When initial results skew toward low-quality sources, re-search with the `site:` operator (e.g. `site:arxiv.org`) targeting authoritative domains.
